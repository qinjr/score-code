...
class SCORE_v3(SliceBaseModel):
    def __init__(self, feature_size, eb_dim, hidden_size, max_time_len, 
                obj_per_time_slice, user_fnum, item_fnum, neg_sample_num = NEG_SAMPLE_NUM):
        super(SCORE_v3, self).__init__(feature_size, eb_dim, hidden_size, max_time_len, obj_per_time_slice, user_fnum, item_fnum, neg_sample_num)
        user_1hop_gat = self.gat(self.user_1hop, self.user_1hop, self.target_item)
        item_1hop_gat = self.gat(self.item_1hop, self.item_1hop, self.target_user)

        user_1hop_seq, item_2hop_seq, user_1hop_wei, item_2hop_wei, agg1 = self.co_attention_v1(self.user_1hop, self.item_2hop)
        user_2hop_seq, item_1hop_seq, user_2hop_wei, item_1hop_wei, agg2 = self.co_attention_v1(self.user_2hop, self.item_1hop)
        ...
        # get agreement of each time slice
        self.agg = tf.nn.softmax((agg1 + agg2))
        inp = tf.concat([user_side_final_state, item_side_final_state, self.target_item, self.target_user, self.agg], axis=1)
    ...
    def co_attention_v1(self, seq1, seq2):
        ...
        seq2 = tf.layers.dense(seq2, seq2.get_shape().as_list()[-1], activation=tf.nn.relu, use_bias=False)
        product = tf.matmul(seq1, tf.transpose(seq2, [0, 1, 3, 2]))
        aggrement = tf.reduce_sum(product, axis=[2, 3])
        ...
        return seq1_result, seq2_result, seq1_weights, seq2_weights, aggrement
